...
export function createService() {
  ...
  function state(instanceId) {
    ...
    if (!selected || !instances.exists(instanceId)) {
      fail("FGPM_RUNTIME_INSTANCE_MISSING", "Transform authority has no state for an instance.", { instanceId });
    }
    return selected;
  }
  ...
  function restore(fragment) {
    if (fragment?.protocol !== "fgpm.state-fragment/1" || fragment.semanticSchema !== stateSchema
      ...
      fail("FGPM_STATE_FRAGMENT_UNSUPPORTED", "Transform Authority cannot restore the supplied state fragment.", {
    ...
    for (const entry of fragment.payload.transforms) {
      if (!instances.exists(entry.instanceId) || !validVector(entry.translation)
        ...
        fail("FGPM_STATE_FRAGMENT_INVALID", "A transform-state entry is invalid for the restored world.", { entry });
      }
    ...
    if (missing.length > 0) {
      fail("FGPM_STATE_FRAGMENT_INCOMPLETE", "The transform-state fragment omits live world instances.", { missing });
    }
  ...
  function commitBatch(batch) {
    if (!["fgpm.transform-command-batch/1", "fgpm.transform-command-batch/2"].includes(batch?.schema)
      || batch.channel !== "runtime.transforms.commands"
    ...
    if (batch.expectedRevision !== revision || (lastCheckpoint && batch.checkpoint.tick <= lastCheckpoint.tick)) {
      fail("FGPM_TRANSFORM_BATCH_STALE", "Transform Authority rejected output for a stale checkpoint.", {
    ...
    for (const buffer of batch.buffers) {
      ...
      if (!["fgpm.runtime-command-buffer/1", "fgpm.runtime-command-buffer/2"].includes(buffer?.schema)
        || buffer.channel !== batch.channel
      ...
      for (const command of buffer.commands) {
        ...
        target.translation[axis] = Number((command.operation === "set-axis"
          ? command.value : target.translation[axis] + command.value).toFixed(6));
    ...
    const current = snapshot();
    return freeze({
      schema: "fgpm.transform-batch-commit/1",
      authority: serviceId,
  ...
  return {
    async activate(context) {
      ...
      const write = freeze({
        submit: (command) => {
          if (command?.schema !== "fgpm.transform-command/1" || typeof command.instanceId !== "string"
          ...
          return freeze({
            schema: "fgpm.transform-commit/1",
            authority: serviceId,
